...
public class DisableChaosAddressCommandAuthorizerTests
{
    [Fact]
    public void BuildPolicy_AddsAuthenticationRequirement()
    {
        // Arrange
        var authorizer = new DisableChaosAddressCommandAuthorizer();
        var chaosAddressId = Guid.NewGuid();
        var command = new DisableChaosAddressCommand(chaosAddressId);
    ...
    public void BuildPolicy_WorksWithDifferentSubdomainIds()
    {
        // Arrange
        var chaosAddressId = Guid.NewGuid();
        var authorizer1 = new DisableChaosAddressCommandAuthorizer();
        var authorizer2 = new DisableChaosAddressCommandAuthorizer();
        var command1 = new DisableChaosAddressCommand(chaosAddressId);
        var command2 = new DisableChaosAddressCommand(chaosAddressId);
