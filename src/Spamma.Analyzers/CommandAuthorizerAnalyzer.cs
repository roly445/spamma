...
///
/// NOTE: This analyzer is currently disabled because custom local project analyzers
/// (using OutputItemType="Analyzer") are not properly loaded by the .NET build system.
/// To re-enable, this would need to be packaged as a NuGet package or integrated differently.
/// </summary>
...
public sealed class CommandAuthorizerAnalyzer : DiagnosticAnalyzer
{
        ...
        : CSharpSyntaxWalker
    {
        ...
        private static bool IsAuthorizerType(INamedTypeSymbol typeSymbol)
        {
            ...
            {
                if (baseClass.Name.Contains("AbstractRequestAuthorizer") ||
                    baseClass.Name.Contains("RequestAuthorizer"))
                {
                    return true;
                }
        ...
        private static ITypeSymbol? ExtractAuthorizedCommandType(INamedTypeSymbol authorizerType)
        {
            ...
            while (baseClass is not null)
            {
                if (baseClass.IsGenericType &&
                    (baseClass.Name.Contains("AbstractRequestAuthorizer") ||
                     baseClass.Name.Contains("RequestAuthorizer")) &&
